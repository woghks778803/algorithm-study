...
def bfs(start):
    ...
    while len(deq) > 0:
        ...
        # 이전 부모 타입 값 알아야함
        if v_list[visit_point-1] == False:
            v_list[visit_point-1] = True
...
for _ in range(K):
    ...
    v_list = [False for _ in range(V)]
    type_list = [None for _ in range(V)]
